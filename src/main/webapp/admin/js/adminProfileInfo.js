...
async function profileData(){
	const firstName = document.getElementById('firstname');
	    ...
	    try {
	        let response = await fetch("http://localhost:8080/complaint-management/AdminInfo", {
	            method: 'POST',
	            headers: {
...
document.getElementById('btn').addEventListener('click', async function (event) {
    ...
    console.log("Submitted values:", firstName, lastName, phoneNumber);
    ...
    try {
        const response = await fetch("http://localhost:8080/complaint-management//AdminUpdate", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
